...
# ```
After faceting, you can see how sporadic Sub-Saharan Africa is. This region was probably the reason for those spike from 2001 - 2007 with all regions. In additional Africa is prone to food insecuritys due to shortages, poverty and conflict. 
In East Asia & Pacific we see the price drop for a few years but then start to pick up again. (find why). This looks the same for the Middle East & North Africa, but we do not have enough data to see if this was a trend. 
...
After looking at price its good to look at inflation for each region to identify stable and unstable regions. To do this I created a box plot.
#```{r}
#plot_region_box(rice_avg_region)
#```
No surprise that Sub-Saharan Africa looks like the most unstable region with the most significant outliers. After investigating the data I found that these outliers Liberia in 06 Rwanda in 2015.
Interestingly Latin America & Caribbean has the smallest quartile range which means it is one of the most stable regions for rice price.

So far we have found that from 2006 to 2007 there was a spike in rice food prices worldwide. We also know that Sub-Saharan Africa has the most high and inconsistent prices. This is the same fore inflation where we see that Sub-Saharan Africa is the least stable and Latin America & Caribbean is the most stable.

To further investigate these findings we will analyse the price of rice in every country available. 
#```{r}
...
Next lets take a look at how inflation vaires from country to country. 
#```{r}
...
To see all the countries plotted together we can use a box plot once again.
#```{r}
...
#```
As you can see in every country imported local and not listed follow the same trend with just a slight increaser or decrease in price, 

But lets see if inflation for import and rexport has a significant difference. 
#```{r}
plot_import_countries_inflation(rice_countries_and_imports)
#```
It looks like Chad and Mali have an identical distrobution for both Import and Not Listed which leads me to believe that rice that I catorgized as not listed may be imported rice. ADitionally for both these countries Local rice has a much less stable price which makes since because the growing season. Mali has three Oct-Dec, Dec-Jan and May-Jul and then Chad 
