...
Middle_East_North_Africa_price_matrix
East_Asia_Pacific_price_matrix
Europe_Central_Asia_price_matrix
South_Asia_price_matrix
Latin_America_Caribbean_price_matrix
East_Asia_Pacific_price_matrix
Sub_Saharan_Africa_price_matrix
...
<center>
  ...
  price for each day and plotting a line chart.
  # ```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_group_price(maize_world_avg)
  # ```
  
  
  overall inflation of rice,  histogram
  # ```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_group_inflation_hist(maize_world_avg)
  # ```
  
  
  different regions of the world same line chart.
  # ```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_region_price(maize_avg_region)
  # ```
  
  regions by faceting each chart.
  # ```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_region_price_facet(maize_avg_region) #all four regions
  plot_region_price_facet_A(maize_avg_region)
  plot_region_price_facet_B(maize_avg_region)
  # ```
  
  inflation for each region box plot.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  price in every country 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_price_county(maize) #all countries
  facet_price_county_by_Middle_East_North_Africa(maize)
  facet_price_county_by_East_Asia_Pacific(maize) #2 countries
  facet_price_county_by_Latin_America_Caribbean(maize) #8 countries
  facet_price_county_by_Europe_Central_Asia(maize) #2 countries
  facet_price_county_by_Sub_Saharan_Africa(maize) #26 countries
  facet_price_county_by_Sub_Saharan_Africa_lib_nig(maize) #Took out liberia and nigeria 
  #```
  
  inflation vaires from country to country.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  
  facet_inflation_county_by_East_Asia_Pacific(maize) 
  facet_inflation_county_by_Latin_America_Caribbean(maize)
  facet_inflation_county_by_Europe_Central_Asia(maize)
  
  facet_inflation_county_by_Sub_Saharan_Africa(maize) #to many countries so I split the facet into 2 graphs
  facet_inflation_county_by_Sub_Saharan_Africa_A(maize)
  facet_inflation_county_by_Sub_Saharan_Africa_B(maize)
  #```
  
  countries inflation box plot 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_country_box(maize) #outlier in Ethiopia
  #```
  
  
  price to survive. NEED TO CALC 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  price_to_survive_plot_line(rice_price_to_survive)
  price_to_survive_plot_bar(rice_price_to_survive)  
...
<center>
  ...
  price in every country 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_price_county_by_Middle_East_North_Africa(sorghum) #only 1 country
  facet_price_county_by_Latin_America_Caribbean(sorghum) #only 1 country
  facet_price_county_by_Sub_Saharan_Africa(sorghum) #19 countries
  facet_price_county_by_Sub_Saharan_Africa_lib_nig(sorghum) #Took out liberia and nigeria 
  #```
  
  inflation vaires from country to country.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_inflation_county_by_Latin_America_Caribbean(sorghum)
  facet_inflation_county_by_Middle_East_North_Africa(sorghum)
  facet_inflation_county_by_Sub_Saharan_Africa(sorghum) #to many countries so I split the facet into 2 graphs
  facet_inflation_county_by_Sub_Saharan_Africa_A(sorghum)
  facet_inflation_county_by_Sub_Saharan_Africa_B(sorghum)
  #```
  
  countries inflation box plot. Outlier in Ethiopia
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_country_box(sorghum)
  #```
  
  
  price to survive.  create
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  price_to_survive_plot_line(rice_price_to_survive)
  price_to_survive_plot_bar(rice_price_to_survive)  
...
<center>
  ...
  regions by faceting each chart.
  # ```{r, echo=FALSE, message=FALSE, warning=FALSE}
  plot_region_price_facet(beans_avg_region) #all 5 
  plot_region_price_facet_B(beans_avg_region) # first 3
  plot_region_price_facet_A(beans_avg_region) # second 2
  # ```
  ...
  price in every country 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_price_county_by_East_Asia_Pacific(beans) #2 countries
  facet_price_county_by_Middle_East_North_Africa(beans) #5 countries
  facet_price_county_by_Latin_America_Caribbean(beans) #6 countries
  facet_price_county_by_South_Asia(beans) #1 countries
  facet_price_county_by_Europe_Central_Asia(beans) #2 countries
  facet_price_county_by_Sub_Saharan_Africa(beans) #20 countries
  #```
  
  inflation vaires from country to country.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_inflation_county_by_East_Asia_Pacific(beans)
  facet_inflation_county_by_Latin_America_Caribbean(beans)
  facet_inflation_county_by_South_Asia(beans)
  facet_inflation_county_by_Europe_Central_Asia(beans)
  facet_inflation_county_by_Middle_East_North_Africa(beans)
  facet_inflation_county_by_Sub_Saharan_Africa(beans) #to many countries so I split the facet into 2 graphs
  facet_inflation_county_by_Sub_Saharan_Africa_A(beans)
  facet_inflation_county_by_Sub_Saharan_Africa_B(beans)
...
<center>
  ...
  price in every country 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_price_county_by_Middle_East_North_Africa(millet) #1 country
  facet_price_county_by_Sub_Saharan_Africa(millet) #12
  #```
  
  inflation vaires from country to country.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_inflation_county_by_Middle_East_North_Africa(millet)
  facet_inflation_county_by_Sub_Saharan_Africa(millet) #to many countries so I split the facet into 2 graphs
  facet_inflation_county_by_Sub_Saharan_Africa_A(millet)
  facet_inflation_county_by_Sub_Saharan_Africa_B(millet)
  #```
...
<center>
  ...
  plot_region_price_facet_B(oil_avg_region)
  plot_region_price_facet_A(oil_avg_region)
  # ```
  ...
  price in every country 
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  facet_price_county_by_East_Asia_Pacific(oil)
  facet_price_county_by_Middle_East_North_Africa(maize)
  facet_price_county_by_Latin_America_Caribbean(oil)
  facet_price_county_by_South_Asia(oil)
  facet_price_county_by_Europe_Central_Asia(oil)
  facet_price_county_by_Sub_Saharan_Africa(oil)
  facet_price_county_by_Sub_Saharan_Africa_lib_nig(oil) #Took out liberia and nigeria 
  #```
  
  inflation vaires from country to country.
  #```{r, echo=FALSE, message=FALSE, warning=FALSE}
  
  facet_inflation_county_by_East_Asia_Pacific(oil)
  facet_inflation_county_by_Latin_America_Caribbean(oil)
  facet_inflation_county_by_South_Asia(oil)
  facet_inflation_county_by_Europe_Central_Asia(oil)
  facet_inflation_county_by_Middle_East_North_Africa(oil)
  
  facet_inflation_county_by_Sub_Saharan_Africa(oil) #to many countries so I split the facet into 2 graphs
  facet_inflation_county_by_Sub_Saharan_Africa_A(oil)
  facet_inflation_county_by_Sub_Saharan_Africa_B(oil)
